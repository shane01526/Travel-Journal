...
if database_url == 'sqlite:///travel_journal.db' and os.environ.get('SUPABASE_DB_PASSWORD'):
    database_url = f"postgresql://postgres:{os.environ.get('SUPABASE_DB_PASSWORD')}@db.rtfdfsvqigdiadnffcxs.supabase.co:5432/postgres"
...
logger.info(f"使用資料庫: {database_url.split('@')[0]}@***")
...
if __name__ == '__main__':
    app.run(debug=False, host='0.0.0.0', port=int(os.environ.get('PORT', 5000)))# app.py (完整更新版)
from flask import Flask, render_template, request, jsonify, session, redirect, url_for, flash, make_response
from flask_sqlalchemy import SQLAlchemy
from werkzeug.security import generate_password_hash, check_password_hash
from datetime import datetime
